...
<body>
				...
				<div class="contact">
					<div class="contact-grids">
						<div class="col-md-8 contact-form">
							<h3>friend</h3>
							<div
								style="width: 80%; height: 400px; overflow: scroll; border: 1px solid;"
								id="output"></div>
							<br />
							<div style="text-align: left;">
							<span class="emotion">表情</span></p>
								<input id="username" name="username" type="hidden"
									value="${userBean.userName}"> <input id="in"
									name="message" value="" onkeyup="listener(event)" type="text"
									style="width: 80%; height: 60px; border: 1px solid;"> <br />
								
									
								<input  value="发送" class="sub_btn" type="button" id="send"/>
							</div>
						</div>
					</div>
				</div>
	...
	<script type="text/javascript">
	var localChatKey="friend_key"+"${friend}"
	...
	ws.onmessage = function(message) {
		...
		if(!isArrayFn(obj)){
			...
			writeToScreenInit(message.data);
		}
	};
	...
	*/
	...
	function writeToScreenInit(message){
		 var arrayObj=JSON.parse(message);
		 for(var i=0;i<arrayObj.length;i++){
			 var message=arrayObj[i];
			 writeToScreenObj(message);
		}
	...
	function writeToScreen(message) {
        ...
        var obj=JSON.parse(message);
        writeToScreenObj(obj);
		
	}
	function writeToScreenObj(obj){
		var jsonArray=localStorage.getItem(localChatKey);
    	var objArray=JSON.parse(jsonArray);
		var html;
		if(obj.id==${userBean.id}){//自己发送的消息
